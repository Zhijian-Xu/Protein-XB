...
def find_inter_halogenBond(PDB,halogen_chain,halogen_resi,halogen_name,cf):
    cmd.load(pdb_path+PDB+'.cif.gz')
    cmd.remove('solvent')
    halogen_bond_info=[]
    halogen_pairs=cmd.find_pairs('polymer.protein and c. {} and resi {} and name {}'.format(halogen_chain,halogen_resi,halogen_name),'polymer.protein and not c. {} and (e. N+O+S or (r. phe+tyr+his+trp and name CG))'.format(halogen_chain),cutoff=cf)
    if halogen_pairs:
        for halogen_atom,pair_atom in halogen_pairs:
            ...
            if name_Y=='CG':
                ...
                if (min_dis<=halogen_dis<=max_dis) and (halogen_alphaang<X_pi_alpha) and (halogen_thetaang>X_pi_theta):
                    halogen_bond_info.append(((PDB,chain_X,resn_X,resi_X,name_X,symbol_X,index_X),(chain_Y,resn_Y,resi_Y,name_Y,'Pi',index_Y,str(max_dis),str(halogen_dis),str(halogen_alphaang)+'/'+str(halogen_thetaang))))
...
def multi_find_inter_halogenBond(param_ls):
    return find_inter_halogenBond(param_ls[0],param_ls[1],param_ls[2],param_ls[3],param_ls[4])

all_inter_halogenbond=[]
pool=Pool(128)
inter_halogenbonds=pool.map(multi_find_inter_halogenBond,input_param_ls)
pool.close()
pool.join()
for info in inter_halogenbonds:
    all_inter_halogenbond.extend(info)

print('done')

with open(materials_path+'halogen_bond_chainsinfo3+1_231208_pi80-100.csv','w')as f:
    ...
    for info_X,info_Y in all_inter_halogenbond:
